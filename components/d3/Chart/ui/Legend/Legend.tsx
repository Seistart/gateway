...
export const Legend = ({ show, ...rest }: Props) => {
  ...
  return (
    <>
      ...
      > */}
      <div
        className={`ml-4 mt-6 bg-white py-3 shadow-md transition-all duration-300 ease-out ${show ? "max-w-[10rem] p-3" : "max-w-0"} relative right-0 z-20 flex h-[auto] flex-1 overflow-hidden`}
      >
        <div className="h-4 text-gray-900" ref={contentRef}>
          ...
          <ul className="mt-1 flex flex-col gap-2 p-0 text-sm first:mt-0">
            {tags.map((tag) => (
              <LegendItem type={tag}>{tag}</LegendItem>
            ))}
          </ul>
        </div>
      </div>
  ...
  return (
    <li className="flex list-none flex-row gap-4">
      <div className="flex h-10 flex-none items-center">
        <FaCircle size={24} color={getColor(type).darkColor} />
      </div>
